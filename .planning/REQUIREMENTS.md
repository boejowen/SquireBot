...
- [ ] **LNX-05**: A `.tar.gz` ships the static binary + README + a systemd **user** unit + `install.sh` (installs to `~/.local/bin`, enables the unit for autostart, runs first-time `--setup`); `minio/selfupdate` auto-update works on Linux with the linux asset in the update manifest.
...
| LNX-05 | Phase 25 — Linux Watcher | Pending |
