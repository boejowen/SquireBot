...
**Test 1 PASS** (views render, sticky Char, filter/sort). **Test 2 PASS** (gear OK/OTHER/MISSING + spell KNOWN/MISSING badges over live data). **Test 3 = not a bug** — `didYouMean` is whole-name, edit-distance ≤2 (ported verbatim from v1, the WEB-03 oracle); the test query `frozn` is ~15 edits from any full item name so it correctly yields no suggestion. A proper near-miss (e.g. `Rusty Helmet` → `Rusty Helm`, 2 edits) does suggest. Working as designed. **Test 4 — WAS AN ISSUE, NOW FIXED** (`f12ad9b`, deployed): item rows had no wiki link because the URL came from the empty `item_master.wiki_url`; now the link is derived from the item name at render time (`wikiUrlFor`, per FEATURES.md/WEB-04) so every item links. **Test 5 — empty-state was CORRECT** (UI-SPEC copy verbatim; `bank` is scoped to a designated `is_bank_toon`, none set = P15/P16). To demo the populated bank, `Demoknight` was flagged `is_bank_toon=1` (demo-only, reversible) → bank now shows 12 rows + the `Coin: not yet recorded` affordance (`coin: null`, no fabricated `0pp`). All 5 satisfied (1/2 pass · 3 works-as-designed · 4 fixed `f12ad9b` · 5 empty-state-correct + populated). Tests 4 + 5 await the user's visual re-confirm after a hard-refresh of https://squirebot.quest.]
