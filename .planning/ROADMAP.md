...
- **999.37** ✅ **RESOLVED (code-complete, pending next deploy) — quick `260624-mdp`, 2026-06-24** (commit `470b670`; `EvictionPreviewHandler` now mirrors the action's sentinel + owner-floor guards, 3 regression tests). Eviction *preview* lacks the floor/sentinel guards the evict *action* enforces (Phase 36 code-review **WR-01**, non-blocking) — `EvictionPreviewHandler` (`internal/backendsrv/webadmin/eviction.go`) returns an owner's roster without applying the owner-floor / `GuildSentinelOwnerID` guards that `EvictHandler`/`EvictOwnerTx` do, so a peer/officer can preview a floor-protected owner's characters (read-only info-disclosure asymmetry). Pre-existing — Phase 36 only narrowed the preview's *content*. Fix: mirror the action's floor/sentinel guards in the preview handler. Backend-only; no data risk.
- **999.38** ✅ **RESOLVED (code-complete, pending next deploy) — quick `260624-mdp`, 2026-06-24** (commit `71fed2c`; `recentOtherSharerSubquery` gains a live-steward `EXISTS` filter, drift-lock re-scoped, +`TestEvictOwnerTx_RepointSkipsEvictedSharer`). Eviction steward-repoint can target an already-evicted sharer (Phase 36 code-review **WR-02**, cosmetic) — the D-03 `recentOtherSharerSubquery` repoint in `EvictOwnerTx` picks the most-recent *other* `cross_owner_write` uploader without excluding evicted/in-grace owners, so a surviving shared char can be repointed to an already-evicted steward. Cosmetic only (`owner_id` is non-binding; the char survives `is_removed=0` and is never archived) but it's the stale-attribution outcome D-03 meant to avoid. Fix: add `AND EXISTS (SELECT 1 FROM character c2 WHERE c2.owner_id = a.attempting_owner_id AND c2.is_removed = 0)` to the subselect (note: diverges it from `sharedCharPredicate` → re-scope the `TestRepointSubquery_LocksPredicateToSharedPredicate` drift-lock). Backend-only.
- **999.39** ✅ **RESOLVED (code-complete, pending next deploy) — quick `260624-mdp`, 2026-06-24** (commits `d98da18`+`470b670`; `store/linking.go`, `auth/store.go`, and the `webadmin/eviction.go` floor bridge exclude the sentinel by id). Label-bridge lookups can resolve the reserved `'guild'` sentinel label (Phase 35 code-review **IN-02**, astronomically unlikely) — owner-by-label `SELECT`s in `store/linking.go`, `auth/store.go`, and the `eviction.go` label bridge can match the sentinel owner (id 1000000, label `guild`, seeded by migration 00015) IF a guildie's Discord username / mint `--owner` arg were ever literally "guild". Newly reachable since the reserved label was introduced. Fix: exclude `o.id <> store.GuildSentinelOwnerID` (or `label <> 'guild'`) from those label SELECTs. Backend-only; defense-in-depth.
