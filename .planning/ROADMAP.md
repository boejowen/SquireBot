...
- [x] **Phase 41: Character paper-doll — compaction + portrait photo** — compact the paper-doll layout toward the in-game inventory-window idiom (reclaiming the empty portrait dead space) AND add an optional per-character portrait photo upload shown in the portrait area. CHARUI-01, CHARUI-02 (new per-character image reference + migration + upload path + read-API). ✅ **COMPLETE + DEPLOYED LIVE + browser-smoke APPROVED 2026-07-16.** 2 plans — 41-01 backend (`792a56b`/`4635326`/`201f096`: migration 00019 `character_portrait` side table + `store/portrait.go` blob CRUD + `authorizePortraitWriteTx` assignee-or-officer gate under-tx [bank/bot officer-only] + base64-in-JSON upload [`sniffImageType` PNG/JPEG/WebP-only, SVG/GIF rejected, ≤256KB cap] + DELETE + raw-byte serve [content-type from sniff + nosniff] + `has_portrait`/`portrait_updated_at` on CharacterInventory + 3 RequireSession routes) / 41-02 web (`f8d80fc`/`e6530ef`/`5e06519`: `.paperdoll` gap 24→16 + drop the 260px `.doll` floor + square portrait frame + portrait `<img>` onImgError→⚔ fallback + `PortraitControl.svelte` Add/Change/Remove [is_mine|isOfficer client gate, canvas downscale] + api.ts). Verifier **PASSED 17/17**; code-review **0 BLOCKER** (1 HIGH WR-01 + 5 LOW/NIT). ⚠ Browser-smoke found a **cross-origin CORS-DELETE bug** (the remove preflight was blocked — `Access-Control-Allow-Methods` lacked DELETE) → fixed-forward `c08817d` + regression test; code-review HIGH **WR-01** (upload body unbounded before the decoded cap) → fixed-forward `d3088a9` (`http.MaxBytesReader` + test). ✅ **DEPLOYED LIVE 2026-07-16** (backend + web together; goose applied **00019** on boot → prod schema **v18→v19**, character_portrait table present; two backend fix-forward redeploys, binary-only); 5 LOW/NIT → backlog **999.46**. `go test ./internal/backendsrv/...` green; web check 0/399→409 tests/build OK. Watcher untouched → no `v*` tag.
...
- **999.46** Phase 41 code-review LOW/NITs (non-blocking, deferred 2026-07-16 — `41-REVIEW.md`; the review's 1 HIGH **WR-01** [unbounded upload body before the decoded cap] was fixed-forward `d3088a9` + the CORS-DELETE browser-smoke bug fixed-forward `c08817d`): **IN-01 (LOW)** `handleWriteError`'s `_op` param is dead, so a failed *Remove* shows the "Couldn't save the photo" copy — thread `_op` to pick save/remove wording (`web/src/lib/components/PortraitControl.svelte`); **IN-02 (LOW)** the optimistic `portraitOverride` in `InventoryWindow.svelte` never clears for the same char, so a later server re-fetch can keep a stale `?v=` — clear the override on the next inventory load; **IN-03 (NIT)** migration `00019`'s comment says "idempotent" but uses a bare `CREATE TABLE` (no `IF NOT EXISTS`) — goose versions it so it's fine, just tighten the comment; **IN-04 (NIT)** the client `too_large`/`invalid_image` error branches in `PortraitControl` are effectively unreachable on the happy path (client validates first) — kept as server-truth defense, document or drop; **IN-05 (NIT)** `PortraitResult.updated_at` is optional in `api.ts` but the POST handler always returns it — make it required. All web + one migration-comment; a future `/gsd-quick`.
