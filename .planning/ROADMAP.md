...
- ✅ **v2.5** — Ownership Cleanup — Phases 35–36 (opened + feature-complete 2026-06-22; promotes backlog 999.35/36; owner-less guild banks/bots + shared-character-safe eviction; backend+web, no `v*` tag) — OWN-01..04 ALL COMPLETE + DEPLOYED LIVE (prod schema v15); ready for milestone audit/close

## Phases

### v2.5 — Ownership Cleanup (Phases 35–36) — ✅ FEATURE-COMPLETE 2026-06-22

Promotes backlog 999.35 + 999.36 (deferred from quick `260621-u6j`). One schema migration (00015, Phase 35); watcher untouched → no `v*` tag. Both phases shipped + DEPLOYED LIVE (prod schema v14→v15 on the Phase 36 deploy boot). Ready for `/gsd-complete-milestone v2.5`.
