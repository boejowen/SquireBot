...
Stand up the self-hosted backend so guildie watchers have a live, authenticated place to upload to again: an Oracle Cloud Always Free (Ampere A1, ARM64) box behind Caddy auto-HTTPS, a SQLite schema under `goose` forward-only migrations, per-guildie bearer-token auth, and the ingest endpoint that atomically replaces a character's rows. Foundation for everything after.
...
- **D-01:** **Plan 11-01 is a time-boxed 1-day PocketBase spike** (do it first). Pass criteria — all four must hold: (a) models `owner`/`character`/`inventory_item`/`spellbook_entry` + empty dimension tables; (b) supports the per-guildie bearer-token ingest contract via a custom route/hook doing atomic full-snapshot replace; (c) can host the P12 in-process enrichment cron via Go hooks (PocketBase-as-framework, `pocketbase.New()`); (d) runs on Oracle ARM64. All four pass → adopt PocketBase. Any hard blocker → hand-rolled Go fallback. Host (Oracle Always Free) + DB (SQLite) stand either way. **Capture the verdict in an 11-01 SUMMARY / appended CONTEXT note.**
- **D-02 (hand-rolled fallback shape):** Go 1.24, stdlib `net/http` ServeMux (1.22+ method+pattern routing — no router dependency), pure-Go SQLite driver **`modernc.org/sqlite` (NO cgo)** so the binary cross-compiles `GOOS=linux GOARCH=arm64` from the Windows dev box into a single static binary (mirrors the watcher's ethos).
...
- **D-11:** **Nightly cron `sqlite3 .backup`** (consistent snapshot) → upload to **Oracle Object Storage (Always Free 10 GB)**. Documented restore = pull latest snapshot, place file, start binary (`goose.Up` no-ops). Litestream (continuous replication to Object Storage) noted as a future upgrade if the nightly RPO proves too loose.
- **D-12:** Oracle instance = **`VM.Standard.A1.Flex`, 1 OCPU / 6 GB** (well within the 4 OCPU / 24 GB Always Free allotment), **US home region**. User provisions (see Prerequisites).
...
- `.planning/explorations/website-milestone/01-backend-hosting.md` — backend architecture + the PocketBase flag (read "Hetzner/$55" as "Oracle Always Free/$0"; "Postgres" as "SQLite")
