...
verification: `npx vitest --run` → 261/261 pass (holders.test.ts 11/11, including the regression: catalog id 19450 want + inventory id 14536 holders, same name → both holders returned / "In guild"). `npm run check` (svelte-check) → 0 errors. `npm run build` → succeeds. DEPLOYED LIVE 2026-06-06 (npm build → scp web/build → chmod a+rX; assets serve as `text/javascript`, site HTTP 200 — no blank-screen; commit 9985fe1).
files_changed:
...
verification (backend): Falsified against the old id-join (new tests FAIL: "Ant's Potion price = {has:false …}") then confirmed PASS with the CTE. `go build ./...` clean; `go vet ./internal/backendsrv/...` clean; `go test ./...` all packages OK (store, compute, readapi green incl. updated fixtures). DEPLOYED LIVE 2026-06-06 (cross-compile → scp → `systemctl restart`; schema unchanged at v8, no migration; `is-active=active`, startup_errors=0; commit 0a169f3). Live price coverage jumped **156 → 984** priced inventory rows (~9% → ~56%); the remaining ~789 are genuinely absent from the PigParse catalog by name (P99-only / no-trade / quest items).
