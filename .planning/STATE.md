...
Plan: 25-02 COMPLETE (2/3 plans) — Linux runtime impls (credstore / eqfind / onboarding / CLI)
Status: 25-02 done — the `!windows` runtime seams are filled: 0600-file credstore under `$XDG_CONFIG_HOME/squirebot/guild_code` (atomic tmp+rename; token never logged), WINE-prefix eqfind walk (knownpaths split + relaxed heuristic guard + bounded WINEPREFIX/~/.wine/Lutris/Bottles/Proton walk), CLI stdin onboarding replacing the unsupported placeholders, a NEW additive `app.RunSetup` (RunApp signature byte-unchanged, D-07), and `squirebot --setup`/`--status` (status never prints the code). `GOOS=linux ... go build ./...` + `GOOS=linux go vet ./...` green; linux closure still CGO-free (systray=0, sqweek=0); Windows build + host `go test ./...` green (additive guarantee). Next: `/gsd-execute-phase 25` plan 25-03 (packaging + auto-update: release.yml linux build/tarball/systemd-unit/install.sh + manifest OS-asset selection, LNX-05). v2.2 Track 1 remains SHIPPED LIVE; Track 2 (22–23) parked, invite-gated.
