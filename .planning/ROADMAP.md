...
- [x] **Phase 38: Catalog-wide enrichment + icon coverage** — widen enrichment beyond held items to the full PigParse Blue catalog (politefetch-paced) and backfill icons for every item whose wiki page provides one, with a maintainer-visible coverage diagnostic for the residue. ENRICH-14, ENRICH-15. The "missing item images" bug + the full-catalog data the SEARCH-06 toggle reads. ✅ **COMPLETE 2026-06-25** — 1 plan / 2 tasks; verifier PASSED 5/5 (goal-backward); code-review 0 BLOCKER / 0 HIGH (MD-01 steady-state coverage diagnostic + MD-02/LO-01/LO-02 fixed-forward `6d6588f`, NIT-02 → backlog 999.42); D-04 → **Option A** (catalog-only rows admitted to `item_master` keyed by PigParse id, only for unheld normalized names; catalog arm excludes ids already in `item_master`). **NO new migration** (00016 stays last); icons ride the existing freshness short-circuit; D-03 coverage `slog` reads true `item_master` state. `go build`/`vet`/`test ./...` green (30 pkgs); watcher untouched, no `v*` tag. ⚠ migration-free but **NOT yet deployed** (rides the next backend deploy, alongside 00016 from P37); the D-04 prod collision probe (expected ≈0) is a pre-deploy checklist item.
...
**⚠ STATUS (2026-06-25): RE-OPENED — D-04 storage reversed id-keyed → NAME-KEYED; RE-PLAN IN PROGRESS.** First build (id-keyed Option A: catalog-only rows into `item_master` by PigParse id + an `item_id NOT IN item_master` collision guard) shipped code-complete (verifier 5/5, review 0-blocker; on origin/master through `85c3f03`) but was **NEVER deployed**. The pre-deploy prod probe disproved the research's ≈0-collision assumption — **60 raw id-collisions / 43 genuinely-unheld catalog items the guard silently drops** (~1% of catalog; no correctness risk, but a permanent coverage hole). User held the deploy and ratified a **name-keyed re-plan**: a separate `catalog_enrichment(norm_name PK,…)` table (Option B), `item_master` stays held-only by EQ id, migration **00017**, covers all 4,343 with no drop. The Option-A code is being REPLACED. See `38-CONTEXT.md` ⚠ D-04 REVERSED block + `38-DISCUSSION-LOG.md`. The 37+38 deploy is held until this lands.
...
- **999.42** Phase 38 code-review residual NIT (non-blocking, deferred 2026-06-25 — `38-REVIEW.md`): **NIT-02** — a "keep in sync with the SQL dedup key" reminder comment on the `normEnrich` test helper (`store/itemids_test.go`); cosmetic and largely moot (the helper already carries a mirror-of-the-SQL comment). The two MEDIUMs (MD-01 steady-state coverage diagnostic, MD-02 docstring over-claim) + both LOWs (LO-01 double-count, LO-02 untested id-keying) were **fixed-forward in Phase 38 itself** (commit `6d6588f`); NIT-01 (residue cap mismatch) was dissolved by that fix. Backend-only; a future `/gsd-quick`.
