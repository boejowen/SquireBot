...
- [x] **Phase 8: Test Infra + Persistence + Docs Backfill** — JSDOM in vitest + 4 net-new sidebar inline-JS test files (Search, Eviction, Bank-Coin, Char-Info; Admin-Mgmt covered by Phase 7's trigger-call test → 5/5 sidebars under vitest); SEARCH recent-MRU migrated from CacheService to `PropertiesService.getUserProperties()`; 8 retroactive Phase 3+4 SUMMARY.md files backfilled. **SHIPPED 2026-05-12**; 336/336 vitest green; verifier PASSED (5/5 must-haves); 4 reqs closed (TEST-01, TEST-02, SEARCH-05, DOC-04). Latent regression in `showSearchSidebar.test.ts` fixed inline (commit `ec68e3f`). Schema gates untouched.
...
**Plans**: 4 plans
  - [x] 08-01-test-infra-jsdom-PLAN.md -- vitest.config.ts (`environment: 'jsdom'`) + jsdom@^24 devDep + mountSidebar helper + PropertiesService.getUserProperties mock alias (TEST-01). Wave 1. **SHIPPED 2026-05-12** (commits `53433d1` + `48c47a8` + `98b0fb1` + `4801284`; 327→327 baseline GREEN under JSDOM; SUMMARY at `.planning/phases/08-test-infra-persistence-docs/08-01-SUMMARY.md`).
  - [x] 08-02-sidebar-inline-js-tests-PLAN.md -- 4 net-new `__tests__/*Sidebar.inline.test.ts` files (Search, Eviction, Bank-Coin, Char-Info) + `buildSidebarHtml` exports on 4 sidebar triggers + REQUIREMENTS.md TEST-02 wording correction (Theme Picker → Admin-Mgmt + deferral note) + mountSidebar hardening (indirect-eval + nested-script extraction) (TEST-02). Wave 2. **SHIPPED 2026-05-12** (commits `e7320cc` + `c0a6606` + `1e4eb26` + `be37ae2` + `1a2fc21` + `77d4fcb` + `ff6a01e`; 328→336 GREEN; SUMMARY at `.planning/phases/08-test-infra-persistence-docs/08-02-SUMMARY.md`).
  - [x] 08-03-search-mru-properties-migration-PLAN.md -- `searchIndex.ts` MRU swapped from `CacheService.getDocumentCache()` to `PropertiesService.getUserProperties()` (no dual-write per D-06); `CACHE_TTL_SECONDS` retained for the 2 surviving per-`inv:Char` enrichment cache sites; new `vi.useFakeTimers()` 25-min persistence test (SEARCH-05). Wave 1. **SHIPPED 2026-05-12** (commits `a326187` + `a21e448` + `4e67360`; +1 TTL-elapse test; SUMMARY at `.planning/phases/08-test-infra-persistence-docs/08-03-SUMMARY.md`).
  - [x] 08-04-doc-backfill-summaries-PLAN.md -- 8 retroactive SUMMARY.md files (Phase 3 plans 01–04 + Phase 4 plans 01–04) following the Phase 5 template byte-for-byte; every `metrics.commits` SHA resolves under `git cat-file -e`; v1.0 milestone audit's documentation-debt line item retired (DOC-04). Wave 1. **SHIPPED 2026-05-12** (commits `3fecfa6` + `8f71a9f` + `be9cef9`; SUMMARY at `.planning/phases/08-test-infra-persistence-docs/08-04-SUMMARY.md`).
**UI hint**: yes
**Ship gate**: `clasp push` of the apps-script bundle to the dev workbook + green CI + milestone retrospective (v1.0.1 ship) — deferred to milestone close
...
| v1.0.1 | 3 | 12/12 | 🚧 In progress (3/3 phases shipped; milestone close pending retrospective) | — |
...
| 8. Test Infra + Persistence + Docs Backfill | 4/4 | ✅ Shipped — verifier PASSED 5/5 must-haves; 336/336 vitest green | 2026-05-12 |
