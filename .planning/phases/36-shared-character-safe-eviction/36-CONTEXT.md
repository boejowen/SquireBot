...
**In scope:** narrowing `store.EvictOwnerTx` (and the preview/picker it feeds) so a character stewarded by the evicted owner survives the eviction when at least one *other* guildie has uploaded it; keeping the guild-code revoke and the grace/restore/archive lifecycle intact for the chars that ARE removed.
...
### Preview / picker parity (success criterion 3)
- **D-04:** `PreviewEviction(X)` MUST list exactly the chars that will actually be removed (the D-02 narrowed set), and should surface that shared chars are preserved + the code is revoked, so the officer confirm UI matches the action. `ListEvictableOwners` keeps listing any owner with ≥1 **live** character (so an owner who only stewards shared chars is still evictable — to revoke their code), but the count/preview must not imply shared chars will be deleted. The **restore path is unaffected**: shared chars are never put in grace by an eviction, so they never appear in `ListRestorableOwners` / `RestoreOwnerTx`.

### Migration need
- **D-05:** Phase 36 ships with **NO schema migration** — detection is computed on read from `audit_log` + `character`. Schema stays **v15**. Backend-only; **watcher untouched**; **no `v*` tag** (consistent with v2.3/v2.4/v2.5 framing — the milestone's one migration was `00015` in Phase 35).
...
- `internal/backendsrv/webadmin/eviction.go` — `EvictHandler` / `PreviewEviction` / `RestoreHandler` and `mapEvictionErr` (the HTTP surface; confirm the narrowed preview flows through unchanged and no new error path is needed).
