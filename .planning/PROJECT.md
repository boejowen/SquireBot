...
**v2.0 progress — Phase 13 (Watcher Re-Target + Onboarding) COMPLETE (2026-05-30):** the watcher now uploads to the backend HTTP API (raw UTF-8 POST); all Google OAuth/Sheets/Picker machinery deleted; native "paste your guild code" onboarding shipped via the auto-updater; binary 57% smaller + Google-secret-free, SemVer pre-release auto-update twin in place. WATCH-08/09/10/11 satisfied.

**v2.0 progress — Phase 14 (Web Frontend) COMPLETE (2026-05-30):** the read UI is rebuilt as a static SvelteKit app over a versioned Go read API. The backend exposes `/api/v1/views/{view,gear_check,spell_check,bank}` + `/api/v1/meta` (public per D-04, exact-origin CORS), composing a Go reimplementation of the four v1 view builders — **WEB-02 parity proven by Go table-tests translated from the v1 vitest fixtures**. The greenfield `web/` SvelteKit (adapter-static) frontend ships one reusable filterable/sortable `DataGrid` instantiated 4× (sticky leading Char column + header), cross-character fuzzy search with inline "did you mean?", hover/tap rich-HTML item tooltips (HTML-escaped + scheme-validated wiki links), and a 5-theme `[data-theme]` EQ aesthetic system (velious default, self-hosted fonts — no Google runtime dependency). Verified human_needed: 6/6 must-haves code-verified (Go whole-repo suite + 60 web tests green; `npm run build` emits the static site), 0 Critical code-review findings (the one security Warning WR-01 — wiki-URL scheme allow-list — fixed in `d14e4ab`). BACKEND-05 + WEB-01..05 code-complete. **DEPLOYED LIVE 2026-05-30:** the site is up at `https://squirebot.quest` — served by Caddy on the same VPS at the apex (deploy decision: switched from the planned Cloudflare Pages `app.` subdomain to apex-on-Caddy — one control plane, valid Let's Encrypt cert), and the rebuilt P14 binary runs on the box (`/api/v1/*` → 200, CORS = exactly-one apex origin, DG-1 clean). Per-character views are empty until guildies' re-targeted (P13) watchers upload; the UI/theme/empty-state UAT is walkable now (`14-HUMAN-UAT.md`). Next: `/gsd-plan-phase 15` (Discord login + admin write forms).
...
- **P11 — Backend foundation + ingest API:** Hetzner Cloud VPS (US) + Caddy auto-HTTPS; SQLite schema with `goose` migrations; per-guildie bearer-token auth; the upload-receiving endpoint. *Front-loaded so guild data flows again ASAP.*
