...
- **999.32** Single-bank-toon invariant for the char-meta form (Phase 16 code-review **MD-01**) — `SetCharMetaTx` (`internal/backendsrv/store/charmeta.go`) enforces no uniqueness on `is_bank_toon`, but `compute.Bank` assumes exactly one bank toon; flagging 2+ silently merges bank-view rows. Fix: when setting `is_bank_toon=true`, clear it on all other characters in the same tx (matches v1's single-value `_meta.bank_toon_name`) + a regression test. Bundle the two Lows from the same review: **LO-01** align the `level` JSON-null contract (Go `int64` can't emit `null` vs TS `number | null`); **LO-02** empty-name success copy on read-back failure. Full findings in `16-REVIEW.md`. Quick fix path: `/gsd-code-review-fix 16` before the phase is archived, else a `/gsd-quick`.
